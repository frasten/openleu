...
int EvilBlade(struct char_data *ch, int cmd, char *arg,struct obj_data *tobj, int type)
{
        ...
        obj ; obj = obj->next_content) 
   {
     if( obj->item_number >= 0 && 
         obj_index[obj->item_number].func == CASTVF EvilBlade )
     {
       /* I am on the floor */
   ...
   for (holder = real_roomp(ch->in_room)->people; holder ; 
     ...
     for (obj = holder->carrying; obj ; obj = obj->next_content) 
     {
       if( obj->item_number >= 0 && obj_index[obj->item_number].func && 
           obj_index[obj->item_number].func != CASTVF board )
       {
         /*held*/
     ...
     if (holder->equipment[WIELD]) 
     {
       if( holder->equipment[WIELD]->item_number >= 0 &&
           ...
           obj_index[holder->equipment[WIELD]->item_number].func != CASTVF board )
       {
         /*YES! I am being held!*/
...
int GoodBlade(struct char_data *ch, int cmd, char *arg,struct obj_data *tobj, int type)
{
   ...
   {
     if( obj->item_number >= 0 && 
         obj_index[obj->item_number].func == CASTVF GoodBlade ) 
     {
       /* I am on the floor */
       for (joe = real_roomp(ch->in_room)->people; joe ; 
            joe = joe->next_in_room) 
             ...
             int type) 
{
  ...
  {         /* not FIGHTING */
    ...
    if( IS_SET( ch->specials.act, ACT_GREET ) ) 
    {
      ...
      { 
        mudlog( LOG_CHECK, "paladin trying greet someone..." );
...
int ChurchBell(struct char_data *ch, int cmd, char *arg, struct room_data *rp, int type)
{
  if (cmd==PULL)
  {
    ...
    if(!strcmp("rope",buf)) 
    {
      send_to_all("The bells of Alma Civitas's church sound 'GONG! GONG! GONG! GONG!'\n");
      return(TRUE);
    } /* end strcmpi() */
    return(FALSE);    
  } /* end pull */
...
if (type == EVENT_DEATH && ch->in_room == SLAV_LIVE_ROOM) {
  struct char_data *v;
        ...
        send_to_all("The chruch bells of Alma Civitas sound off in a chorus of happiness!\n");        
    ...
    else
    {
      ...
      if( number( 1, 10 ) == 1 ) 
      {
        act( "$n seems enraged to the point of exploding!", FALSE, ch, 0, 0,
             TO_ROOM );
        act( "Your anger whells up inside, you fling yourself at your opponet!",
      ...
      if( pObject) 
      {
        if( !IS_OBJ_STAT( pObject, ITEM_NODROP ) || IS_IMMORTAL( ch ) )
        {
          ...
          if( pObject->obj_flags.cost >= MINVALUETOHEAL )
          {
            ...
            spell_dispel_magic( IMPLEMENTOR, mob, ch, NULL );
            spell_remove_poison( IMPLEMENTOR, mob, ch, NULL );
            spell_heal( IMPLEMENTOR, mob, ch, NULL );
          }
        }
        else 
        {
...
#define IsLegionarioV( ch ) ( (ch)->nr == 12014 || (ch)->nr == 12015 || \
                              (ch)->nr == 12016 || (ch)->nr == 12017 || \
...
/*****************************************************************************
  Bilancia permette la creazione della bilancia (#1605) eseguendo un JOIN
  fra la pipe (#1602) a cui e` attaccata la procedura, il disc(#1603) ed
  il piece of iron (#1604).
*****************************************************************************/

#define BALANCE_PIPE  1602
#define BALANCE_DISC  1603
#define BALANCE_PIECE 1604
#define BALANCE       1605
#define ALA           1606
              ...
              int type )
{
  ...
  struct obj_data *pObj[ 3 ];
  int i, bWrongObj = FALSE;
  struct obj_data *pPipe = NULL, *pPiece = NULL, *pDisc = NULL;

  if( type == EVENT_COMMAND && cmd == CMD_JOIN )
  {
    ThreeArgumentInterpreter( arg, achArgs[ 0 ], achArgs[ 1 ], achArgs[ 2 ] );
    for( i = 0; i < 3; i++ )
    {
      if( achArgs[ i ][ 0 ] )
      {
        pObj[ i ] = get_obj_in_list_vis( ch, achArgs[ i ], ch->carrying );
        if( pObj[ i ] == NULL || pObj[ i ]->item_number < 0 )
          return FALSE;
        switch( obj_index[ pObj[ i ]->item_number ].iVNum )
        {
        case BALANCE_PIPE:
          pPipe = pObj[ i ];
          break;
    ...
    if( pPipe && bWrongObj )
    {
      act( "Non riesci a trovare il modo di farli combaciare.", FALSE, ch,
           0, 0, TO_CHAR );
      return TRUE;
    }
    else if( pPipe && pDisc && pPiece )
    {
      struct obj_data *pBalance = read_object( BALANCE, VIRTUAL );
      if( pBalance )
      {
        act( "Unisci i tre oggetti e ricostruisci $p.", FALSE, ch, pBalance, 0,
             TO_CHAR );
        act( "$n unisce tre oggetti e ricostruisce $p.", TRUE, ch, pBalance, 0,
             TO_ROOM );
        obj_from_char( pPipe );
        extract_obj( pPipe );
        obj_from_char( pPiece );
        extract_obj( pPiece );
        obj_from_char( pDisc );
        extract_obj( pDisc );
        obj_to_char( pBalance, ch );
      }
      else
        act( "Influenze diaboliche interferiscono con il tuo tentativo. "
             "Contatta un Dio.", FALSE, ch, 0, 0, TO_CHAR );
        
      return TRUE;
    }
    else if( pPipe && ( pDisc || pPiece ) )
    {
      act( "$p e $P combaciano, ma non sembrano formare qualcosa di completo.",
           FALSE, ch, pPipe, pDisc != NULL ? pDisc : pPiece, TO_CHAR );
      act( "$n prova ad unire $p e $P senza ottenere nulla di completo.",
           TRUE, ch, pPipe, pDisc != NULL ? pDisc : pPiece, TO_ROOM );
      return TRUE;
    }
    else if( pPipe && !pDisc && !pPiece )
    {
      act( "Con cosa vuoi unire $p ?", FALSE, ch, pPipe, 0, TO_CHAR );
      return TRUE;
    }
  }
  return FALSE;
               ...
               struct char_data *pMob, int iType )
{
  char *aszMessStart[] =
  {
    ...
    "trovarla'.\n\r",
    "Qualcuno qui vicino borbotta 'Mannaggia! mi sa che l'ha presa "
    "qualcuno!'.\n\r"
  };
  ...
  if( AWAKE( pMob ) )
  {
    ...
    {
      int iExitN;
      struct room_data *pRoomNear;
    
      switch( pMob->generic )
      {
       case EZ_START:
        for( iExitN = 0; iExitN < 6; iExitN++ )
        {
          if( exit_ok( EXIT( pMob, iExitN ), &pRoomNear ) )
          {
            struct char_data *pCharTarget;
            for( pCharTarget = pRoomNear->people; pCharTarget; 
                 pCharTarget = pCharTarget->next_in_room )
            {
              if( isname( GET_NAME( pCharTarget ), "pindus" ) )
              {
                send_to_room( aszMessStart[ number( 0, 2 ) ], 
                              pCharTarget->in_room );
                if( !number( 0, 2 ) )
                  pMob->generic = EZ_CERCA;
                break;
              }
            }
          }
        }
        if( get_char_room_vis( pMob, "pindus" ) )
        {
          do_say( pMob, "Pindus! L'Avatar! Mi cerchi la mia bilancia ?", 0 );
          pMob->generic = EZ_PLEASE;
          return TRUE;
        }
        break;
       case EZ_CERCA:
        do_gossip( pMob, "Qualcuno ha visto la mia bilancia ?", 0 );
        pMob->generic = EZ_GOSSIP;
        return TRUE;
        break;
       case EZ_GOSSIP:
        do_gossip( pMob, aszMessPray[ number( 0, 2 ) ], 0 );
        if( !number( 0, 2 ) )
          pMob->generic = EZ_START;
        return TRUE;
        break;
       case EZ_WAITING:
        if( get_char_room_vis( pMob, "pindus" ) )
        ...
        break;
       case EZ_PLEASE:
        do_say( pMob, "Per favore ?", 0 );
        do_emote( pMob, "guarda Pindus sbattendo le lunghe ciglia.", 0 );
        pMob->generic = EZ_WAITING2;
        return TRUE;
        break;
       case EZ_WAITING2:
        if( !get_char_room_vis( pMob, "pindus" ) )
        {
          pMob->generic = EZ_WAITING3;
          return TRUE;
        }
        break;
       case EZ_WAITING3:
        if( get_char_room_vis( pMob, "pindus" ) )
        {
          do_say( pMob, "Pindus! L'hai trovata ?", 0 );
          pMob->generic = EZ_WAITING2;
          return TRUE;
        }
        break;
       default:
        mudlog( LOG_SYSERR, 
               "Invalid pMob->generic (%d) in Ezmerelda (spec_procs3.c)",
               pMob->generic );
        pMob->generic = EZ_START;
        break;
      }
    }
    else if( iType == EVENT_COMMAND )
    {
      if( iCmd == CMD_GIVE )
      {
        if( isname( GET_NAME( pChar ), "pindus" ) )
        {
          char szArg1[ MAX_INPUT_LENGTH ], szArg2[ MAX_INPUT_LENGTH ];
          argument_interpreter( szArg, szArg1, szArg2 );
          if( get_char_room_vis( pChar, szArg2 ) == pMob )
          {
            struct obj_data *pObject;
            if( ( pObject = get_obj_in_list_vis( pChar, szArg1, 
                                                 pChar->carrying ) ) != NULL )
            {
              if( pObject->item_number > 0 )
              {
                if( obj_index[ pObject->item_number ].iVNum == BALANCE_PIPE ||
                    obj_index[ pObject->item_number ].iVNum == BALANCE_DISC ||
                    obj_index[ pObject->item_number ].iVNum == BALANCE_PIECE )
                {
                  act( "$n da $p a $N.", TRUE, pChar, pObject, pMob, 
                       TO_NOTVICT );
                  act( "dai $p a $N.", TRUE, pChar, pObject, pMob,
                       TO_CHAR );
                  do_say( pMob, "No, potrebbe essere un pezzo, ma solo con "
                                "questo non ci faccio nulla", 0 );
                  act( "$N restituisce $p a $n.", TRUE, pChar, pObject, pMob,
                       TO_NOTVICT );
                  act( "$N ti restituisce $p.", FALSE, pChar, pObject, pMob,
                       TO_CHAR );
                }
                else if( obj_index[ pObject->item_number ].iVNum == BALANCE )
                {
                  do_say( pMob, "L'hai trovata! Grazie, davvero. Adesso "
                               "posso tornare a cucinare!", 0 );
                  do_action( pMob, GET_NAME( pChar ), 23 );
                  do_action( pMob, GET_NAME( pChar ), 49 );
                  ...
                  obj_from_char( pObject );
                  obj_to_char( pObject, pMob );

                  if( ( pObject = read_object( ALA, VIRTUAL ) ) != NULL )
                  {
                    do_say( pMob, "Tieni, portalo a Benem come prova della "
                                  "tua generosita`", 0 );

                    act( "$N da $p a $n.", TRUE, pChar, pObject, pMob, 
                         TO_NOTVICT );
                    act( "$N ti da $p.", TRUE, pChar, pObject, pMob,
                         TO_CHAR );
                  
                    obj_to_char( pObject, pChar );
                  }
                  else
                  {
                    mudlog( LOG_PLAYERS, "Pindus ha completato la quest!" );
                  }
                }
                else
                {
                  ...
                  act( "dai $p a $N.", TRUE, pChar, pObject, pMob,
                       TO_CHAR );
                  do_say( pMob, "No, questo proprio non sembra la mia "
                                "bilancia.", 0 );
                  act( "$N restituisce $p a $n.", TRUE, pChar, pObject, pMob,
                       TO_NOTVICT );
                  act( "$N ti restituisce $p.", FALSE, pChar, pObject, pMob,
      ...
      else if( iCmd == CMD_TELL || iCmd == CMD_ASK )
      {
        char szArg1[ MAX_INPUT_LENGTH ];
        szArg = one_argument( szArg, szArg1 );
        if( isname( GET_NAME( pChar ), "pindus" ) &&
            get_char_room_vis( pChar, szArg1 ) == pMob )
        {
          if( pMob->generic > EZ_WAITING )
          {
            do_say( pMob, "E` una di quelle bilance fatta da un'asta a cui e` "
                          "attacato un piatto da una parte e su cui scorre un "
                          "peso", 0 );
            do_action( pMob, "", 36 );
            do_say( pMob, "Deve avermela presa qualcuno.", 0 );
          }
          else
          {
            do_say( pMob, "Pindus, la mia bilancia. Ho perso la mia bilancia. "
                          "Me la cerchi ?", 0 );
            pMob->generic = EZ_PLEASE;
          }
          return TRUE;
...
{  
  if( nType == EVENT_COMMAND )
  {
    if( nCmd != CMD_ENTER )
    {
      ...
      act( "Questa zona e` avvolta da una impenetrabile nebbia divina.",
           FALSE, pChar, 0, 0, TO_CHAR );
      act( "Ti sembra di sentire una voce che ti avverte di non cercare "
           "altre vie di ingresso.", FALSE, pChar, 0, 0, TO_CHAR );
      return TRUE;
    }
    else
    {
      ...
      {
       case 100:
        if( !strcmp( szArg, "sussurro" ) )
        {
          MoveOne( pChar, 2, FALSE );
          return TRUE;
        }
        break;
       case 4210:
        if( !strcmp( szArg, "sussurro" ) )
        {
          MoveOne( pChar, 1, FALSE );
          return TRUE;
        }
        break;
       default:
        mudlog( LOG_ERROR, 
               "Non dovrebbe poter arrivare qui. BlockWay(spec_procs3.c)" );
        break;
      }
    }
  }
