...
int hit_limit(struct char_data *ch)
{
  ...
  if (!IS_NPC(ch))
  {
    ...
    age2(ch, &ma);
    max = (ch->points.max_hit) + (graf(ma.year, 2,4,17,14,8,0,-10));
  } 
  else
     ...
     /* Hitpoint gain pr. game hour */
{
  ...
  if( IS_AFFECTED( ch, AFF_POISON ) )
  {
    ...
    if( affected_by_spell( ch, SPELL_SLOW_POISON ) )
      dam /= 4;
    if( damage( ch, ch, dam, SPELL_POISON ) == VictimDead )
      return 0;
  }
  ...
  if( IS_AFFECTED2( ch, AFF2_HEAT_STUFF ) )
  {
    ...
    if( damage( ch, ch, dam, SPELL_HEAT_STUFF ) == VictimDead )
      return 0;
  }
  ...
  /* Class/Level calculations */
  /* non-warrior types get slower hps re-gen */
  if( IS_PC( ch ) && !HasClass( ch, CLASS_WARRIOR | CLASS_PALADIN | 
      ...
      if( IS_PC( ch ) ) 
      {
        ...
        for( i = MAGE_LEVEL_IND; i < MAX_CLASS; i++ )
        {
              ...
              GET_LEVEL( ch, i ) < RacialMax[ chrace ][ i ] )
          {
            ...
            {
              send_to_char( "Devi passare alla tua gilda prima di guadagnare "
...
void gain_exp_regardless(struct char_data *ch, int gain, int iClass)
{
  ...
  if (!IS_NPC(ch)) 
  {
    if (gain > 0) 
    {
      ...
      {
        if (i > GET_LEVEL(ch,iClass)) 
        {
          send_to_char("You raise a level\n\r", ch);
          /*GET_LEVEL(ch,iClass) = i;*/
