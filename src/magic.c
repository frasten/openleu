        ...
        switch( number( 1, 7 ) )
        {
         case 1:
          ...
          if (!saves_spell(tmp_victim, SAVING_SPELL))
          {
            ...
            if( MissileDamage( ch, tmp_victim, dam, SPELL_PRISMATIC_SPRAY ) !=
                AllLiving )
              return;
          }
          break;
         ...
         case 4:
          act("$N is hit by a green shaft of light!",FALSE,ch,0,tmp_victim,TO_ROOM);
          cast_poison(level,ch,"",SPELL_TYPE_SPELL,tmp_victim,NULL);
          break;
         case 5:
          act("$N is hit by a blue shaft of light!",FALSE,ch,0,tmp_victim,TO_ROOM);
          cast_paralyze(level,ch,"",SPELL_TYPE_SPELL,tmp_victim,NULL);
          break;
         case 6:
          act("$N is hit by a indigo shaft of light!",FALSE,ch,0,tmp_victim,TO_ROOM);
          cast_feeblemind(level,ch,"",SPELL_TYPE_SPELL,tmp_victim,NULL);
          break;
         case 7:
          act("$N is hit by a violet shaft of light!",FALSE,ch,0,tmp_victim,TO_ROOM);
