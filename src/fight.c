...
void die(struct char_data *ch,int killedbytype)
{
  ...
  for(i=0;i<MAX_CLASS;i++) 
  {
    if( GET_LEVEL(ch,i) > 1 ) 
    {
      if( GET_EXP(ch) < ( titles[i][ (int)GET_LEVEL(ch, i) ].exp / fraction ) ) 
      {
        send_to_char( "\n\r\n\rWARNING WARNING WARNING WARNING WARNING\n\r",
                      ch);
  ...
  for( f = k->followers; f; f = f->next )
  {
    if( IS_AFFECTED( f->follower, AFF_GROUP ) &&
        ( f->follower->in_room == ch->in_room ) && IS_PC( f->follower ) &&
          GetMaxLevel( f->follower ) > group_max_level - 25 )
    {
      group_count++;
    }
  }
...
int DamCheckDeny( struct char_data *ch, struct char_data *victim, int type )
{
  ...
  {
    mudlog( LOG_SYSERR, 
            "GET_POS(victim) <= POSITION_DEAD in DamCkeckDeny fight.c" );
    return( TRUE );
  }
  ...
  {
    mudlog( LOG_SYSERR, buf, "damage(,,,%d) called in PEACEFUL room", type );
                       ...
                       int type)
{
  ...
  if (dam >= 0) 
  {
    ...
    if( type >= TYPE_HIT && type != TYPE_SUFFERING && ch != v )
    {
      ...
      {
        damage( v, ch, dam, SPELL_FIREBALL );
      }
    }
    
    update_pos( v );

    // Nel caso qui sotto, il soggetto e` stato ucciso dal fireshield, a
    // meno che, ovviamente, ch non sia uguale a victim.
                            ...
                            int dam, int attacktype )
{
  ...
  if( DoDamage( ch, victim, dam, attacktype ) == SubjectDead )
    return SubjectDead;
                        ...
                        int DistanceWeapon)
{
  ...
  if( ch != victim && !CanFightEachOther( ch, victim ) ) 
  {
    ...
    act( "$n sembra quasi attacarti, ma all'improvviso cambia espressione "
      ...
      { 
        if( GET_HEIGHT(v) < 250 )
        {
          dam += dice( wielded->obj_flags.value[1],
                       wielded->obj_flags.value[2] / 2 );
          send_to_char( "La tua arma sembra meno efficace contro una "
                        "creatura cosi` piccola.\n\r", ch);
        }
        else 
        {
...
DamageResult root_hit( struct char_data *ch, struct char_data *victim, 
                       int type,
                       DamageResult (*dam_func)( struct char_data *, 
                                                 struct char_data *,
                                                 int, int ), 
                       int DistanceWeapon )
{
  ...
  if (HitOrMiss(ch, victim, thaco))
  {
    ...
    {
      return HitVictim(ch, victim, dam, type, w_type, dam_func);
    }
    else
    {
      return MissVictim(ch, victim, type, w_type, dam_func);
    }
  } 
  else
  {
...
DamageResult MissileHit( struct char_data *ch, struct char_data *victim, 
                         int type)
{
  return root_hit( ch, victim, type, MissileDamage, TRUE );
}

DamageResult hit(struct char_data *ch, struct char_data *victim, int type)
{
  return root_hit( ch, victim, type, damage, FALSE );
}
...
void PCAttacks( char_data *pChar )
{
  ...
  if( pChar->equipment[ HOLD ] && !pChar->equipment[ WIELD ] &&
      ITEM_TYPE( pChar->equipment[ HOLD ] ) == ITEM_WEAPON && 
      HasClass( pChar, CLASS_MONK ) ) 
  {
    pTmp2 = unequip_char( pChar, HOLD );
  }
  ...
  if( fAttacks > .01 )
  {
     ...
     * is successful */
    perc = number(1,100);
    if( perc <= (int)( fAttacks * 100.0 ) )
    {
      if( pChar->specials.fighting )
        hit( pChar, pChar->specials.fighting, TYPE_UNDEFINED );
    }
#endif            
...
/****************************************************************************
 * Questa procedura ad (e restituisce) un punteggio a pNewChar in base alle
...
int GetBonusToAttack( struct char_data *pChar, struct char_data *pNewChar )
{
  ...
  else if( pChar->abilities.intel <= 15 ) 
  {
    iIntIdx = 2;
  } 
  else 
...
struct char_data *FindAnyVictim( struct char_data *ch)
{
   ...
   * int <= 12 
   * give higher priority to fighters, clerics, magic users thieves is 
   * inv > 12
   * give higher priority to magic users, fighters, clerics, thieves if 
   * int > 15
   */
...
void BreakLifeSaverObj( struct char_data *ch)
{
  ...
  {
    /* break the object. */
    act( "$p si frantuma in un accecante lampo di luce!", TRUE, ch, 
               ...
               obj_data *missile, int tdir, int max_rng) 
{
  ...
  else 
  {
    ...
    act(buf,TRUE,targ,missile,0,TO_CHAR);
    if( damage( ch, targ, dam, TYPE_RANGE_WEAPON ) == VictimDead )
      targ = 0;
