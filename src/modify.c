...
#define REBOOT_AT1 13
#define REBOOT_AT2 19  /* 0-23, time of optional reboot if -e lib/reboot */
...
void do_edit(struct char_data *ch, const char *arg, int cmd)
{
  ...
  case 4: 
    ...
    dflags=0;
    for(i=0;i < (int) strlen(sdflags);i++) {
      switch(sdflags[i]) {
  ...
  if (*string) {   /* there was a string in the argument array */ 
    if ((int) strlen(string) > room_length[field - 1])        {
      send_to_char("String too long - truncated.\n\r", ch);
      *(string + length[field - 1]) = '\0';
    }
    CREATE(*ch->desc->str, char, strlen(string) + 1);
...
void check_reboot()
{
  ...
  struct tm *t_info;
  FILE *boot;
  static int bBootSequenceStarted = FALSE;
        ...
        (t_info->tm_hour + 1) == REBOOT_AT2 ) && t_info->tm_min > 30 ) 
  {
    ...
    {
      if( t_info->tm_min > 50 && bBootSequenceStarted )
      {
      ...
      else if( t_info->tm_min <= 50 ) 
      {
        bBootSequenceStarted = TRUE;
        if( t_info->tm_min > 49 )
          send_to_all( "$c0015ATTENZIONE! $c0014LeU ripartira` entro un minuto!\n\r");
        else if( t_info->tm_min > 45 ) 
          send_to_all( "$c0015ATTENZIONE! $c0014LeU ripartira` entro 5 minuti.\n\r");
        else if( t_info->tm_min > 40 )
          send_to_all( "$c0015ATTENZIONE! $c0014LeU ripartira` entro 10 minuti.\n\r");
        else if( t_info->tm_min > 30 )
          send_to_all( "$c0015ATTENZIONE! $c0014Leu ripartira` entro 20 minuti.\n\r");
      }
