                       ...
                       struct char_data *victim, struct obj_data *obj)
{
  ...
  /* victim = levd6 damage */
  if( damage(ch, victim, dice(lev,6), SPELL_LIGHTNING_BOLT) != SubjectDead )
  {
    lev--;
                       ...
                       struct char_data *victim, struct obj_data *obj)
{
  ...
  tmp_obj->affected[0].modifier = 4 + 
     ( IS_PC( ch ) ? GET_LEVEL(ch, DRUID_LEVEL_IND) : GetMaxLevel( ch ) ) / 8;
                         ...
                         struct char_data *victim, struct obj_data *obj)
{
  ...
  act("$n raddoppia le sue dumensioni!", TRUE, victim, 0,0, TO_ROOM);
  act("Raddoppi le tue dimensioni!", FALSE,victim,0,0,TO_CHAR);
                         ...
                         struct char_data *victim, struct obj_data *obj)
{
  ...
  if (!IS_AFFECTED(victim, AFF_CHARM) && !IS_AFFECTED(ch, AFF_CHARM))
  {
    ...
    act( "Ti senti attratto da $n.",FALSE,ch,0,victim,TO_VICT);
  }
}
                  ...
                  struct char_data *victim, struct obj_data *obj)
{
  ...
  {
    act( "Alcune radice e liane vengono fuori dal terreno e si arrotolano "
         "intorno ai tuoi piedi!", FALSE, victim, 0,0, TO_CHAR);
    act( "Alcune radice e liane vengono fuori dal terreno e si arrotolano "
         "intorno ai piedidi $n!", FALSE, victim, 0,0, TO_CHAR);
    GET_MOVE(victim)=0;
  }
  else
  {
                    ...
                    struct char_data *victim, struct obj_data *obj)
{
      ...
      !affected_by_spell(victim, SPELL_ARMOR))
  {
    ...
    send_to_char( "La tua pelle assume un aspetto ruvido e legnoso.\n\r", 
                 victim);
    act( "La pelle di $n assume un aspetto ruvido e legnoso.", TRUE, ch, 0, 0, 
         TO_ROOM);
                      ...
                      struct char_data *victim, struct obj_data *obj)
{
  ...
  if (HitOrMiss(ch, victim, CalcThaco(ch, victim)))
  {
    ...
    act( "L'armatura di $N inizia a sfrigolare.\n\r", FALSE, ch, 0, victim, 
         TO_CHAR);
    act( "L'armatura di $N inizia a scfrigolare.\n\r", FALSE, ch, 0, victim, 
         TO_NOTVICT);
