...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
  ...
  if (obj->carried_by) 
  {
    mudlog( LOG_SYSERR, "Obj is carried_by when equip (equip_char)");
    assert(0);
  }
  
  if (obj->in_room!=NOWHERE) 
  {
    mudlog( LOG_SYSERR, "Obj is in_room when equip (equip_char)");
    assert(0);
    return;
  }
  ...
  if( ch->in_room != NOWHERE ) 
  {
    ...
    if( obj->item_number >= 0 &&
        ( obj_index[obj->item_number].func == CASTVF EvilBlade ||
          obj_index[obj->item_number].func == CASTVF NeutralBlade ||
    ...
    if( ItemAlignClash(ch, obj) && (GetMaxLevel(ch) < IMPLEMENTOR)) 
    {
      ...
      {
        mudlog( LOG_SYSERR, 
                "ch->in_room = NOWHERE when equipping char (equip_char)");
        assert(0);
      }
    }
  }  
...
void obj_from_room(struct obj_data *object)
{
  ...
  if( object->in_room <= NOWHERE )
  {
    ...
    {
      mudlog( LOG_SYSERR, 
              "An object was just taken from a char, instead of a room");
      assert(0);
    }
  ...
  else     /* locate previous element in list */
  {
    ...
    {
      mudlog( LOG_SYSERR, "Couldn't find object in room" );
      assert(0);
    }
  }
...
struct obj_data *create_money( int amount )
{
  ...
  if( amount <= 0 ) 
  {
    mudlog( LOG_SYSERR, "Try to create negative money.");
    assert(0);
  }
