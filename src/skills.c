...
void do_track(struct char_data *ch, const char *argument, int cmd)
{
  ...
  {
     code = find_path( ch->in_room, CASTVF_FP named_mobile_in_room, &huntd, -dist, 1);
  } 
  else 
  {
     code = find_path( ch->in_room, CASTVF_FP named_mobile_in_room, &huntd, -dist, 0);
  }
  ...
  else 
  {
    if( IS_LIGHT(ch->in_room) ) 
    {
      ...
      sprintf( buf, "Vedi una traccia della tua preda cha va verso %s\n\r", 
                dirsTo[code]);
      send_to_char(buf,ch);
    } 
    else 
...
int track( struct char_data *ch, struct char_data *vict)
{
  ...
  {
    sprintf( buf, "$c0012Vedi una traccia della tua preda verso %s\n\r", 
             dirsTo[code]);
    send_to_char(buf, ch);
    return(TRUE);
  }
...
int dir_track( struct char_data *ch, struct char_data *vict)
{
  ...
  if (code == -1) {
    if (ch->in_room == vict->in_room) {
      send_to_char("$c0012You have found your target!\n\r",ch);
    } else {
      send_to_char("$c0012You have lost the trail.\n\r",ch);
    }
    return(-1);  /* false to continue the hunt */
  } else {
    sprintf(buf, "$c0012Vedi una debole traccia verso %s\n\r", dirsTo[code]);
    send_to_char(buf, ch);
    return(code);
  }
...
{
  if( HasClass( pChar, CLASS_PALADIN ) && GetMaxLevel( pChar ) < LOW_IMMORTAL )
  {
...
void do_doorbash( struct char_data *ch, const char *arg, int cmd)
{
  ...
  sprintf(buf, "$n carica verso %s", dirsTo[dir]);
  act(buf, FALSE, ch, 0, 0, TO_ROOM);
  sprintf(buf, "Carichi verso %s\n\r", dirsTo[dir]);
  send_to_char(buf, ch);
...
void do_first_aid( struct char_data *ch, const char *arg, int cmd )
{
  ...
  if( IS_PC( ch ) )
  {
    if( HasClass( ch, CLASS_BARBARIAN ) )
      ...
      exp_level = GET_LEVEL( ch, PALADIN_LEVEL_IND );
  ...
  for( dude = real_roomp( ch->in_room )->people; dude; 
    ...
    if( dude->specials.fighting ) 
          ...
          dude != ch && IS_FOLLOWING( ch, dude ) )
      {
        ...
        act( "$n leaps heroically to the front of the battle!", FALSE, ch, 0,
             0, TO_ROOM );
...
if (IS_PC(ch) || IS_SET(ch->specials.act,ACT_POLYSELF))
  ...
  }  else  
  {
    ...
    act( "You fly into battle $N with a holy warcry!", TRUE, ch, 0, dude,
         TO_CHAR);
    if( damage( ch, dude, dam, SKILL_HOLY_WARCRY ) != VictimDead )
    {
      if( !ch->specials.fighting )
